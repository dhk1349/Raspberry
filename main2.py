...
pins=[{'TP':4,'EP':17,'R':16,'G':'7'},{'TP':23,'EP':24,'R':20,'G':8},{'TP':6,'EP':12,'R':21,'G':25}]
...
while True:
   for i in range(len(pins)):
      if status[i]['isFull']=0:
         GPIO.output(pins[i]['R'],GPIO.LOW)
         GPIO.output(pins[i]['G'],GPIO.HIGH)
      else:
         GPIO.output(pins[i]['R'],GPIO.HIGH)
         GPIO.output(pins[i]['G'],GPIO.LOW)
